...
export default function Topbar({ onMenuClick }) {
  ...
  const searchResults = useMemo(() => {
    ...
    const customers = searchData.customers
      .filter((customer) => [customer.name, customer.email, customer.company, customer.phone, customer.country, customer.teamName].some((value) => normalize(value).includes(search)))
      .map((customer) => ({
        id: `customer-${customer.id}`,
        title: customer.name,
        subtitle: [customer.email || customer.company || 'Customer', customer.teamName].filter(Boolean).join(' · '),
        type: 'Customer',
        path: `/contacts/${customer.id}`
      }));

    const deals = searchData.deals
      .filter((deal) => [deal.name, deal.customerName, deal.ownerName, deal.stage, deal.teamName].some((value) => normalize(value).includes(search)))
      .map((deal) => ({
        id: `deal-${deal.id}`,
        title: deal.name,
        subtitle: [deal.customerName, deal.stage, deal.teamName].filter(Boolean).join(' · '),
        type: 'Deal',
        path: '/deals'
      }));

    const tasks = searchData.tasks
      .filter((task) => [task.title, task.customerName, task.userName, task.status, task.teamName].some((value) => normalize(value).includes(search)))
      .map((task) => ({
        id: `task-${task.id}`,
        title: task.title,
        subtitle: [task.customerName, task.status, task.teamName].filter(Boolean).join(' · '),
        type: 'Task',
        path: '/tasks'
      }));
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex min-w-0 items-center gap-3 rounded-xl bg-white px-2 py-2 shadow-card sm:px-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-xs font-bold text-ink/45">{[user?.role || 'User', user?.teamName].filter(Boolean).join(' · ')}</p>
            </div>
          </div>
          <button className="grid h-11 w-11 place-items-center rounded-xl bg-white text-ink shadow-card transition hover:text-clay" type="button" onClick={logout} aria-label="Sign out">
            <LogOut size={18} />
          </button>
        </div>
      </div>
