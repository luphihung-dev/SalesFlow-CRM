  ...
  return (
    <>
      <PageHeader
        ...
        action={canManage ? <button className="inline-flex items-center gap-2 rounded-2xl bg-pine px-5 py-3 text-sm font-extrabold text-cream shadow-soft" onClick={() => { setEditingCustomer(null); setFormError(''); setIsModalOpen(true); }} type="button"><Plus size={18} /> Add Customer</button> : null}
      />
      <ErrorBanner message={error} onDismiss={() => setError('')} />

      <div className="mb-5 flex flex-col gap-3 rounded-[1.5rem] border border-white/70 bg-white/70 p-3 shadow-card md:flex-row">
        <div className="relative flex-1">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-ink/35" size={18} />
          <input className="h-12 w-full rounded-2xl border border-ink/10 bg-white pl-11 pr-4 font-semibold outline-none ring-pine/20 focus:ring-4" placeholder="Search contacts..." value={query} onChange={(event) => setQuery(event.target.value)} />
        </div>
        <select className="h-12 rounded-2xl border border-ink/10 bg-white px-4 font-bold text-ink outline-none" value={status} onChange={(event) => setStatus(event.target.value)}>
          ...
          columns={[
            ...
            ...(canManage || canDelete ? [{ key: 'actions', header: 'Actions', render: (customer) => (
              <div className="flex gap-2" onClick={(event) => event.stopPropagation()}>
                {canManage && <button className="rounded-xl bg-white p-2 text-pine shadow-card" type="button" onClick={() => { setEditingCustomer(customer); setFormError(''); setIsModalOpen(true); }} aria-label="Edit customer"><Edit3 size={16} /></button>}
                {canDelete && <button className="rounded-xl bg-white p-2 text-clay shadow-card" type="button" onClick={() => setDeletingCustomer(customer)} aria-label="Delete customer"><Trash2 size={16} /></button>}
              </div>
            ) }] : [])
          ]}
        />
